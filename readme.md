...
# Penjelasan Workflow Program C++ – Password Strength Checker

Program ini digunakan untuk mengevaluasi kekuatan sebuah password, memberi penjelasan mengenai kekurangan password, dan menghasilkan rekomendasi perbaikan secara otomatis.

---

## 🔍 1. Pemeriksaan Karakter Password

Program menggunakan array `hash[4]` untuk mendeteksi apakah password mengandung jenis-jenis karakter berikut:

- `hash[0]` → angka (0–9)  
- `hash[1]` → huruf kecil (a–z)  
- `hash[2]` → huruf besar (A–Z)  
- `hash[3]` → karakter spesial (!@#$%^&* dll)

Jika nilai index tertentu adalah `0`, artinya password tidak memiliki jenis karakter tersebut.

---

## ⚠️ 2. Memberikan Peringatan Jika Ada Karakter yang Kurang

Program menampilkan saran berdasarkan karakter apa yang tidak ditemukan:

- Jika tidak ada angka → sarankan menambah angka  
- Jika tidak ada huruf kecil → sarankan menambah huruf kecil  
- Jika tidak ada huruf besar → sarankan menambah huruf besar  
- Jika tidak ada karakter spesial → sarankan menambah simbol  

Tujuannya memberikan panduan memperbaiki password yang terlalu sederhana.

---

## 📏 3. Mengecek Panjang Password

Program menghitung kekurangan panjang:


Minimal panjang password adalah **8 karakter**.

Jika kurang dari 8, bagian rekomendasi akan menambahkan karakter tambahan.

---

## 🧪 4. Menentukan Level Kekuatan Password

Jika password **valid** dan **bukan weak**, program menampilkan level:

- **medium**
- **kuat**
- **sangatkuat**

Level ini ditentukan dari kombinasi:
- panjang password  
- variasi jenis karakter  

---

## 🔧 5. Membuat Rekomendasi Password

Jika password termasuk **invalid**, **weak**, atau **medium**, program mencetak:


Program lalu menghasilkan karakter acak:
- angka
- huruf kecil
- huruf besar
- karakter spesial

Rekomendasi dibagi menjadi beberapa kasus:

### **Kasus A — Panjang cukup (≥8), tetapi ada jenis karakter yang kurang**
Program menambahkan karakter yang kurang saja.

### **Kasus B — Panjang kurang dari 8, tetapi semua kategori karakter sudah ada**
Password digandakan sampai panjangnya ≥8.

### **Kasus C — Panjang kurang dari 8 dan ada karakter yang kurang**
Program mengisi kekurangan dengan karakter-karakter yang belum ada hingga panjang mencapai 8.

---

## 🔁 6. Menanyakan Apakah Pengguna Ingin Mengecek Password Lain

Jika password valid dan tidak weak:
...
Program ini:
- Mengevaluasi kekuatan password berdasarkan komponen karakternya  
- Menjelaskan kekurangan password kepada pengguna  
- Memberi rekomendasi password otomatis  
- Menawarkan opsi untuk mengecek password baru  

Program cocok untuk latihan logika C++, validasi input, dan pengolahan string.
